...
const uri = `mongodb+srv://${process.env.USER_DB}:${process.env.USER_PASS}@cluster0.jwhv1v2.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
    try {
        ...
        app.post('/users', async (req, res) => {
            ...
            // you can do this many ways:  (1.email unique, 2.upsert, 2.simple checking)
            const query = { email: user.email };
            const existingUser = await userCollection.findOne(query);
            if (existingUser) {
                return res.send({ message: 'User already exists', insertedId: 0 })
            }
        ...
        app.post('/payments', async (req, res) => {
            ...
            mg.messages
                .create(process.env.MAIL_SENDING_DOMAIN, {
                    from: "Mailgun Sandbox <postmaster@sandboxd2431cb5f6614067b78a70d69546e4c4.mailgun.org>",
                    to: ["mdriazuddinriad5@gmail.com"],
        ...
        app.get('/admin-stats', verifyToken, verifyAdmin, async (req, res) => {
            ...
            // best way to calculate revenue
            const result = await paymentCollection.aggregate([   //pipeline
                {
                    $group: {
                        _id: null,    // _id:null means it takes all the ids from database
                        totalRevenue: {
                            $sum: '$price'
                        }
                    }
        ...
        app.get('/order-stats', verifyToken, verifyAdmin, async (req, res) => {
            const result = await paymentCollection.aggregate([
                ...
                //     $group: {
                //         _id: {
                //             category: '$menuItems.category', name: '$menuItems.name'
                //         },
                //         quantity: { $sum: 1 },
                //         price: { $sum: '$menuItems.price' },
                //     }
